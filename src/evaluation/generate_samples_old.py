...
def validate_exact_bullets(text: str, n: int) -> dict:
    cnt = _count_bullets(text)
    return {"pass": cnt == n, "found": cnt, "expected": n}
...
def build_checks(prompt_item: dict) -> list[dict]:
    """
    Checks can be explicitly specified per prompt item via prompt_item["checks"].
    Otherwise we infer from prompt text.
    """
    # Explicit checks (best practice)
    if isinstance(prompt_item.get("checks"), list):
        return prompt_item["checks"]

    prompt = prompt_item["prompt"]
    checks = []

    n_b = _infer_exact_bullets(prompt)
    if n_b is not None:
        checks.append({"type": "exact_bullets", "n": n_b})

    n_q = _infer_first_n_questions(prompt)
    if n_q is not None:
        checks.append({"type": "first_n_lines_questions", "n": n_q})

    if re.search(r"\breturn\s+only\s+valid\s+json\b", prompt, flags=re.IGNORECASE):
        # Optional: allow per-item schema override via prompt_item["json_schema"]
        schema = prompt_item.get("json_schema")
        checks.append({"type": "json_schema", "schema": schema})
...
def run_checks(prompt_item: dict, cleaned_text: str) -> dict:
    ...
    for chk in checks:
        ...
        if ctype == "exact_bullets":
            r = validate_exact_bullets(cleaned_text, chk["n"])
...
def main():
    ...
    parser.add_argument("--max_new_tokens", type=int, default=128) #256
